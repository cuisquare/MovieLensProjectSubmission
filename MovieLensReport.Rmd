...
The objective of the analysis is to produce a recommendation system for movies using the MovieLens data set. This would work as follow: A given user will rate a series of movies. Then based on this information, as well as information gathered on other users ratings, the program will attempt to estimate which ratings the user would have given to movies they have not yet watched. The highest rated such unseen movies would constitute the recommendation. 

Therefore the objective is to build a model that provides the most accurate estimation of the ratings for movies yet unseen by a user.

## Data preparation and usage {#dataprep}

The data set has been provided and split in a training set (90% of the data) and validation set (10% of the data) without overlap. The validation set was then modified to contain only users and movies which are in the training set. The data removed to ensure this, was then added back to the training set. 

The validation set thus defined will be used to provide "unseen" data in the sense defined in the General Objectives.

Using the training data, a prediction system will be evaluated and tuned. For the purpose of making choices of model type and settings to best predict ratings, an iterative process will be used comparing results obtained by predicting to actual ratings. However the validation data set will not be used to choose the best model. This is because if it was, it would result in overfitting, that is, a model that best fit the data we are considering but might not generalize well to new data. Instead, **only** the training set  will be used both for the purpose of providing tuning (training) data and providing a measure of the model performance. 

In order to be able to have real data to check the accuracy of the model on, the training set will therefore be further split in 10 randomly selected non overlapping sets. Then a subtraining set made up of 9 of the splits (or folds) will be used as actual training set, and the 10th remaining fold used to estimate the accuracy of the model. Because the 10 folds are non overlapping it is possible to repeat this step 10 times, resulting in 10 measures of the model and increasing our confidence that results obtained can be generalised to unseen data. 
...
The smaller RMSE is, the better our recommendation system is likely to be at predicting a user rating for an unseen movie, hence at recommending movies.

In the case of classic 10 folds cross validation, used to determine the best model, the RMSE used to do so should be the average of the RMSEs obtained for each fold. However for speed of execution we have applied a simplified method where only the "worst fold" was used to tune the model (See section [Simplified K Folds cross validation] for details). The goal of checking results over 10 tolds is to further increase the likelihood that the RMSE thus obtained be also reached on future unseen data, compared to using a single partition of the training data and corresponding RMSE.
...
Since it is not possible to know whether this will be reached on the validation data set (As it is unseen data), we will consider in this study that a particular target is reached, when it is *likely* to be reached on unseen data. Practically we decide that this be the case when **all RMSE values over 10 folds are lower than the particular target**. This assumption is deemed reasonable considering that:
...
Of note that it is impossible to know for sure how the model will perform, just estimate how it will perform because it is still possible  that the validation set is so peculiar and the folds themselves so peculiar that the model does not generalize as well as could be hoped. However considering the above considerations, this should be unlikely.
...
The MovieLens data contains a total of `r nb_observations_tot` observations, which have been split as outlined in Section \ref{dataprep} into a training set named edx containing `r nb_observations_edx` observations and a validation set named validation containing `r nb_observations_validation` observations. 
...
In Figure \ref{fig:ratings-spread}, one can see that the most given ratings is 4, and that whole star ratings are more common than half star ones.  It also shows that there are no actual ratings above 5 or below 0.5 which therefore appears to be inherent to the grading interface. It would therefore logically imply that any prediction that states a value below 0.5 (resp  greater than 5) will be improved by rounding up (resp down) to 0.5 (resp 5), and so this effect will be included in the prediction models. 

```{r ratings-spread, fig.cap = "Ratings Spread"}
print(p_ratings_spread)
```

Furthermore, it is interesting to consider what the boundaries (minimum and maximum rating) for individual users. This is because if a given user has never given a rating below 2 for instance, it is likely that it will not happen for a new rating by that user. That is, it is likely that for that particular user, a rating of 2 is the minimum it will give to a movie they do not like. This similarly goes for the high ratings. It would be interested to see the effect of extracting the minimum and maximum ratings for each individual users and apply this as boundaries for a prediction of an unseen movie for that user, that is, if the prediction falls within that user's boundaries so far, keep the prediction, and if the prediction is greater (resp lower) than the top (resp bottom) boundary, change the prediction to the top (resp bottom) boundary for that user. 
...
The best model considered is one that incorporates the following effects: *`r rmse_folds_results_summary$method[index_min_RMSE]`* achieving a median RMSE of *`r rmse_folds_results_summary$median_RMSE[index_min_RMSE]`* with a maximum RMSE of **`r  rmse_folds_results_summary$max_RMSE[index_min_RMSE]`** obtained on `r worst_fold_best_method` i.e. every fold out of the 10 randomly generated Folds on the training data beat the lowest target RMSE = `r rmse_target_25` hereby achieving the objective set in section [Meeting the challenge objectives].

The best model without regularisation incorporates the following effects: `r method_min_RMSE_nonreg` achieving a median RMSE of *`r rmse_folds_results_summary$median_RMSE[index_min_RMSE_nonreg]`* with a maximum RMSE of **`r  rmse_folds_results_summary$max_RMSE[index_min_RMSE_nonreg]`** obtained on `r worst_fold_best_method` i.e. each of the 10 randomly generated Folds on the training data beat the lowest target RMSE = `r rmse_target_25`. 

The following models also achieve the objective set to beat the lowest target RMSE:
...
methods_betterthantarget <- rmse_folds_results_summary %>% 
  ...
  mutate(method_result_string = paste0(method," (max_RMSE=",max_RMSE,")")) %>%
...
The full simplified cross validation results for the models considered can be found in Table \ref{tab:results-table} and Table \ref{tab:results-summary-table}, ordered by descending mean RMSE (increasing mean prediction accuracy) over the 10 folds randomly generated.
...
It also shows that the lowest target RMSE is beaten with a significant margin. 
...
rmse_folds_results_table$`Fold 1` = cell_spec(
  ...
  #color = "gray",
  color = case_when(rmse_folds_results_table$`Fold 1` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 1` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 1` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 1` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_table$`Fold 2` = cell_spec(
  rmse_folds_results_table$`Fold 2`, 
  #color = "gray",
  color = case_when(rmse_folds_results_table$`Fold 2` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 2` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 2` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 2` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_table$`Fold 3` = cell_spec(
  rmse_folds_results_table$`Fold 3`, 
  #color = "gray",
  color = case_when(rmse_folds_results_table$`Fold 3` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 3` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 3` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 3` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_table$`Fold 4` = cell_spec(
  rmse_folds_results_table$`Fold 4`, 
  #color = "gray",
  color = case_when(rmse_folds_results_table$`Fold 4` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 4` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 4` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 4` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_table$`Fold 5` = cell_spec(
  rmse_folds_results_table$`Fold 5`, 
  #color = "gray",
  color = case_when(rmse_folds_results_table$`Fold 5` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 5` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 5` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 5` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_table$`Fold 6` = cell_spec(
  rmse_folds_results_table$`Fold 6`, #color = "gray",
  color = case_when(rmse_folds_results_table$`Fold 6` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 6` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 6` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 6` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_table$`Fold 7` = cell_spec(
  rmse_folds_results_table$`Fold 7`, #color = "gray",
  color = case_when(rmse_folds_results_table$`Fold 7` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 7` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 7` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 7` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_table$`Fold 8` = cell_spec(
  rmse_folds_results_table$`Fold 8`, #color = "gray",
  color = case_when(rmse_folds_results_table$`Fold 8` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 8` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 8` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 8` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_table$`Fold 9` = cell_spec(
  rmse_folds_results_table$`Fold 9`, #color = "gray",
  color = case_when(rmse_folds_results_table$`Fold 9` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 9` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 9` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 9` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_table$`Fold 10` = cell_spec(
  rmse_folds_results_table$`Fold 10`, #color ="gray",
  color = case_when(rmse_folds_results_table$`Fold 10` < rmse_target_25~ "green",
                    rmse_folds_results_table$`Fold 10` < rmse_target_20~ "blue",
                    rmse_folds_results_table$`Fold 10` < rmse_target_15~ "orange",
                    rmse_folds_results_table$`Fold 10` < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
...
rmse_folds_results_summary_table$mean = cell_spec(
  ...
  #color ="gray",
  color = case_when(rmse_folds_results_summary_table$mean < rmse_target_25~ "green",
                    rmse_folds_results_summary_table$mean < rmse_target_20~ "blue",
                    rmse_folds_results_summary_table$mean < rmse_target_15~ "orange",
                    rmse_folds_results_summary_table$mean < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_summary_table$median = cell_spec(
  rmse_folds_results_summary_table$median,
  #color ="gray",
  color = case_when(rmse_folds_results_summary_table$median < rmse_target_25~ "green",
                    rmse_folds_results_summary_table$median < rmse_target_20~ "blue",
                    rmse_folds_results_summary_table$median < rmse_target_15~ "orange",
                    rmse_folds_results_summary_table$median < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
rmse_folds_results_summary_table$max = cell_spec(
  rmse_folds_results_summary_table$max,
  #color ="gray",
  color = case_when(rmse_folds_results_summary_table$max < rmse_target_25~ "green",
                    rmse_folds_results_summary_table$max < rmse_target_20~ "blue",
                    rmse_folds_results_summary_table$max < rmse_target_15~ "orange",
                    rmse_folds_results_summary_table$max < rmse_target_10~ "red",
                     TRUE~ "black") 
  )
...
Next we considered the effect of user boundaries in their ratings. By looking at the minimum and maximum rating given so far by each user, we apply a simpler rule which is that if a prediction is lower than the lowest rating given so far by the user, that lower boundary is predicted instead. Similarly, if a prediction is higher than the highest rating given so far, the prediction is changed to be equal to that higher boundary. This is based on the assumption that each user will have their idea of what is a maximum and minimum rating, keeping to this rather than the full range. By doing so, we yielded a lower RMSE of `r RMSE_movie_userboundary`. As a result we have applied this adjustment for all further models without specifying it in the model descriptions for conciseness. 
...
* **`r final_pred_RMSE`** for the *movie+user+timestamp+indgenres+releaseyear reg* model (best regularised model)
* **`r final_pred_fast_RMSE`** for the *movie+user+indgenres* model (best non regularised model)

As expected, both models do beat the lowest target RMSE of `r rmse_target_25` by a significant margin. 

We observe that the RMSE results is lower than than the highest RMSE observed during 10 folds cross validation for the *movie+user+indgenres* model. However it is higher than than the highest RMSE observed during 10 folds cross validation for the *movie+user+timestamp+indgenres+releaseyear reg* model. This highlights that RMSE achieved in the cross validation stage do not directly translate to general data and therefore that given an objective, it was preferable to derive a model achieving a significantly lower RMSE than our target during simplified cross validation.

The non regularised model results was produced roughly 3 times faster than the regularised model, at the final prediction stage. To this should be added the time required to tune the lambda parameters of the regularised model during simplified cross validation. This highlights that depending on the target RMSE we might want to use a simpler model for speed of execution.
