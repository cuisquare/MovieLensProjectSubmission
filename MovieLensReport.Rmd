...
The challenge in the *HarvardX: PH125.9x Data Science: Capstone* course sets the target RMSE to be reached on the validation set in increasing order of achievement, as follow :
...
## Prediction Model Parameter Tuning methods 

### Simplified K Folds cross validation

Because we try to minimise the RMSE with lambda as a variable, in K folds cross validation we normally would modify the RMSE function to be the average or median RMSE over the K folds using that lambda parameter. This means we would have K times more prediction function evaluation to carry out. Furthermore in some of the models there might be value in using different lambdas for each effects. This can be numerically demanding resulting in a very long run time. 

For this reason, a simpler method was chosen, which is to determine the parameters that minimise RMSE only for one of the folds, then applying those parameters to all remaining folds to obtain the comrresponding RMSEs, to provide a check that the RMSE did overall vary in the same direction across models. That is, that if improvement was brought for the chosen fold on which parameters were tuned, that a similar improvement was also observed on the other folds.

The fold on which to tune the parameters was picked to be the one with the highest RMSE values among all folds, for the model that on average over the K folds gave the lowest RMSE over the models considered thus far. In other words, the "worst" fold for the "best" model. This was because it was considered that the likelihood that the tuning parameters have an impact on improving RMSE that generalises well to unseen data was considered greater in the cases where the RMSE would initially be the highest. 

Because of this simplification it will be necessary to check the assumption that each model bringing improvement in the RMSE over the worst fold, also does bring similar improvement on all other folds.
...
The Simplified Cross Validation Full Results for the models considered can be found in Table \ref{tab:results-table} and Table \ref{tab:results-summary-table}, ordered by descending mean RMSE (increasing mean prediction accuracy) over the 10 folds randomly generated, and located in the [Simplified Cross Validation Full Results] section of the [Appendix].
...
It also provides verification that the simplified cross validation method described in section [Simplified K Folds cross validation] is valid indeed for each model, all 10 folds are within roughly the same RMSE interval with a standard deviation of around 0.00075 and follow the same variation in RMSE from model to model. It is even more clear when drawing each fold RMSE variation across models as a line plot in Figure \ref{fig:rmse-bestmethods-pointline} that tuning on the worst fold does improve the results on all folds in a very similar manner.
...
## Final model prediction on Unseen Data (validation set)

The best model as verified by simplified cross validation is one that applies the effect of movie, user, individual genres, movie release year and rating timestamp with regularisation applied, using separate tuned lambda parameters for each effect expect for the movie and user effect (see section [Multiple Lambda vs Single Lambda Regularisation] for details). The model also includes an adjustment of predicted ratings to be within the allowed boundary for ratings, as well as another adjustment of predicted ratings to be within each user rating range (more details in sections [Rating Rounding / Adjustments])

We can now calculate the RMSE on the unseen data contained in the *validation* set using the whole *edx* set as a training set. To do so, the best model is trained one last time on the whole *edx* (training) set using the lambda parameters previously derived during simplified 10 folds cross validation. We then make a prediction on the *validation* set for which we calculate the RMSE. 

Best model  *movie+user+indgenres+releaseyear+timestamp reg* yields a RMSE of **`r final_pred_RMSE`** which beats the lowest target RMSE of *`r rmse_target_25`* by a significant margin. 

# Conclusion and Future Work

Using a model that incorporated the effects of movie, user, date of rating, individual genres, release year and rating timestamp to account for the variation in ratings, it was possible to reach predictions in movie ratings for unseen data with a RMSE below 0.84, beating the lowest RMSE target of `r rmse_target_25` with a significant margin.
...
As explained in section [Meeting the challenge objectives] we made the choice that a particular model would be considered to be meeting the objective in terms of target RMSE if the worst fold over 10 randomly generated folds on the *edx* set would have RMSE beating that target RMSE with a margin.

We saw in the [Results] section that the best model devised does beat the target RMSE on unseen data. It is however of interest to check whether the other models that also meet our set conditions as being likely to beat the target RMSE actually do, in order to get an idea of how appropriate was our method.

The results for all such models can be found in Table \ref{tab:rmse-validation-best-models}. One can see that all models that do meet the condition set out do end up beating the RMSE target on the unseen data. It therefore acts as an indication that that the worst fold simplified cross validation be sufficient to determine when a particular target RMSE might be beaten on unseen data. 

In particular it is of interest that all 5 models do have a lower RMSE on the unseen data than that obtained on the worst fold during simplified cross validation. This appears to confirm the assumption that the worst fold might be used as a conservative value of the RMSE on unseen data provided that data is likely to be at least as statistically representative as the fold data, which due to the fact that the folds are a smaller subset of the total movielens data than the validation data, is the case.


```{r label = rmse-validation-best-models}
#TODO finish writing

rmse_validation_bestmodels_table <- rmse_validation_bestmodels %>%
  select(method,max_RMSE,RMSE_validation)

names(rmse_validation_bestmodels_table) <- str_remove(
  string = str_replace(
    string = names(rmse_validation_bestmodels_table),
    pattern = "_RMSE",
    replacement = "_RMSESimplifiedXVal"
    ),
  pattern = "_"
  )

kable(x=rmse_validation_bestmodels_table,caption=paste("Validation RMSE for 5 models meeting the target beating objective")) %>%
...
## Simplified Cross Validation Full Results
