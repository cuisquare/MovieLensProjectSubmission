...
Using the best model derived during simplified cross validation, it is now possible to calculate the RMSE on the unseen data contained in the *validation* set. To do so, the *movie+user+indgenres+releaseyear+timestamp reg* model is trained one last time on the whole *edx* (training) set, using the lambda parameters previously derived from simplified cross validation on the worst fold (fold01, with verification on the other 9 folds), then a prediction is made on the *validation* set for which we calculate the RMSE. 

Doing so yields a RMSE of **`r final_pred_RMSE`** for the *movie+user+indgenres+releaseyear+timestamp reg* model which beats the lowest target RMSE of *`r rmse_target_25`* by a significant margin. 
