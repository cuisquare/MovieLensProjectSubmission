...
We observe that the RMSE results is lower than than the highest RMSE observed during 10 folds simplified cross validation for the *movie+user+indgenres* model. However it is higher than than the highest RMSE observed during 10 folds simplified cross validation for the *movie+user+indgenres+releaseyear+timestamp reg* model. This highlights that RMSE achieved in the cross validation stage do not directly translate to general data and therefore that given an objective, it was preferable to derive a model achieving a significantly lower RMSE than our target during simplified cross validation.

The non regularised model results was produced roughly 3 times faster than the regularised model, at the final prediction stage. To this should be added the time required to tune the lambda parameters of the regularised model during simplified cross validation. This highlights that depending on the target RMSE we can use a simpler model for speed of execution.
